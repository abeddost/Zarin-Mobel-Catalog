    ...
    <section className="relative h-svh overflow-hidden border-b border-white/8 bg-dark-0 text-stone-100">
          ...
          >
            {heroItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <article className="h-[8.75rem] w-[6.25rem] overflow-hidden rounded-xl border border-white/15 bg-dark-3/75 shadow-[0_18px_40px_rgba(0,0,0,0.5)] backdrop-blur-[1px] sm:h-[10.5rem] sm:w-[7.5rem] lg:h-[15rem] lg:w-[10.75rem]">
                    <img
                      src={item.image}
                      alt={item.label}
                      loading="eager"
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-[1500px] flex-col px-6 pb-10 pt-28 sm:pb-12 sm:pt-32 lg:px-10 lg:pb-16 lg:pt-36">
        <div className="mx-auto text-center lg:mx-0 lg:max-w-xl lg:text-left">
          <p className="mb-3 text-gold text-[10px] tracking-[0.5em] uppercase">
            Zarin Möbelhaus
          </p>
          <h1 className="font-serif text-[2.4rem] leading-[0.9] tracking-[0.06em] text-stone-100 sm:text-[3.2rem] lg:text-[5.1rem]">
            Tische & Stühle
          </h1>
          <p className="mt-2 font-serif text-[1.15rem] tracking-[0.18em] text-stone-200/90 sm:text-[1.35rem] lg:text-[1.85rem]">
            Kollektion
          </p>
        ...
        <div className="mt-auto flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
          <button
            ...
            className="inline-flex cursor-pointer items-center justify-center border border-gold/75 bg-gold px-8 py-3.5 font-sans text-[0.8rem] uppercase tracking-[0.24em] text-on-gold shadow-[0_14px_34px_rgba(0,0,0,0.46)] transition-colors hover:bg-gold-light focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-light/90 sm:px-10 sm:py-3.5 sm:text-[0.84rem]"
          >
          ...
          <a
            href="/"
            className="text-white/40 hover:text-gold text-[11px] tracking-[0.3em] uppercase transition-colors duration-300"
          >
            ← Polstermöbel
          </a>
        </div>
